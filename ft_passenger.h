    ...
    for (pathIter = pathSet.begin(); pathIter != pathSet.end(); pathIter++) {
        ...
        for (n = 0; n < tmpTrips.size(); n++) {
            ...
            if (n != 0 && n != tmpTrips.size() - 1) {
                TRT = TRT + tripSet[tmpTrips[n]]->getSchDepartureByStop(tmpBoardings[n]) - tripSet[tmpTrips[n - 1]]->getSchArrivalByStop(tmpAlightings[n - 1]) -
                      atof(tmpWalkings[n].c_str());
        ...
        tmpUtility = inVehTimeEqv * IVT + railInVehTimeEqv * railIVT + waitingEqv * (IWT + TRT) + originWalkEqv * ACD + destinationWalkEqv * EGD +
                     transferWalkEqv * TRD + transferPenalty * NTR + scheduleDelayEqv * ScDelay + 60 * FARE / VOT;
